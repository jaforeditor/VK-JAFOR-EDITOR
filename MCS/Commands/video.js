...
module.exports = {
 ...
 role: 0,
 cooldown: 5,
 description: "YouTube থেকে ভিডিও সার্চ ও ডাউনলোড",
 ...
 (err, info) => {
 if (err ||!info) return;
 // তোমার মেইন ফাইল msgCache ইউজ করে
 ...
 try {
 // replyData = cache থেকে আসা ডাটা
 if (event.senderID!== replyData.author) return;

 const index = parseInt(event.body);
 if (isNaN(index) || index < 1 || index > replyData.list.length)
 return react("⚠️");

 const video = replyData.list[index - 1];
